...
#         auc_i = roc_auc_score(y_true_one_hot[:, i], y_scores[:, i])
#         aucs.append(auc_i)
...
#             # Convert labels and predicted outputs to numpy arrays
...
#     auc = compute_multiclass_auc(y_true, y_scores)
...
#     y_true = np.array(y_true)
#     y_scores = np.array(y_scores)
#     auc = compute_multiclass_auc(y_true, y_scores) # auc
#     auc_train.append(auc)
    with open(f'model_result/measurement/acc_train_{model_name}_{date_time}.csv', 'a') as acc_train_file:
        ...
        writer.writerow([epoch, float(accus_train[-1])]) #, float(auc_train[-1])])
    accus_train.append([])
...
#     accuracy_val, auc = eval_fn(model, loader_val)
...
#     auc_val.append(auc)
    with open(f'model_result/measurement/eval_val_{model_name}_{date_time}.csv', 'a') as eval_val_file:
        ...
        writer.writerow([epoch, accus_val[-1]]) #, auc_val[-1]])
    
    model.train() # slowest line
    
    # print evaludation
    output_log_writer(f'[Epoch {epoch+1}] loss={losses[-2]:.2e}')
    output_log_writer(f'[Epoch {epoch+1}] train accu={accus_train[-2]:.2%}') #' train auc={auc_train[-1]:.2%}')
    output_log_writer(f'[Epoch {epoch+1}] validation accu={accus_val[-1]:.2%}') # validation auc={auc_val[-1]:.2%}')
